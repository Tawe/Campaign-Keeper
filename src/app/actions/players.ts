...
export async function createPlayer(input: PlayerInput) {
  ...
  const portraitPath =
    input.portraitUrl?.startsWith("data:")
      ? await savePortraitDataUrl("player", ref.id, input.portraitUrl)
      : null;
...
export async function updatePlayer(playerId: string, input: PlayerInput) {
  ...
  if (input.portraitUrl?.startsWith("data:")) {
    nextPortraitPath = await savePortraitDataUrl("player", playerId, input.portraitUrl);
    await deletePortrait(previousPortraitPath);
  } else if (!input.portraitUrl) {
...
export async function deletePlayer(playerId: string, campaignId: string) {
  ...
  await deletePortrait((doc.data()?.portraitPath as string | null) ?? null);
